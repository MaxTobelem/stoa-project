...
class StoaProjectSupabaseUser extends BaseAuthUser {
  StoaProjectSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> stoaProjectSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StoaProjectSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
